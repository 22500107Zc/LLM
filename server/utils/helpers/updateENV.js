...
async function validDockerizedUrl(input = "") {
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the the platform container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
...
function dumpENV() {
  ...
  const protectedKeys = [
    ...
    // Founder control plane. Only ever set on the operator's own host; losing
    // either would lock the operator out of their own console on the next
