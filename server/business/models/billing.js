...
const Billing = {
  ...
  evaluateAccess: function (record, now = new Date()) {
    ...
    if (!record || record.status === STATUS.UNCONFIGURED) {
      // Billing is not wired up, which says nothing about whether this
      // customer may use the product. Access is the founder's decision, made
      // in the founder console and enforced by request validation.
